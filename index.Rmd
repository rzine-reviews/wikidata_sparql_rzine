...
knitr::include_graphics('figures/triplet_mona_lisa.jpg')
```
<center>
...
- l'option de la clause SELECT, OPTIONNAL, permettant de renvoyer les variables même si elles ne sont pas instanciées (noeuds vides) sur la liste de noms de variables.    
 et les *préfixes* associés aux fragments RDF de Wikidata ([+d'info](https://linkeddatafragments.org/)).
<br/>
...
Dans Wikidata, ce peintre est une entité ? (toute entité Wikidata est désignée par le préfixe `wd:`), ayant pour propriété directe (préfixe `wdt:`) d'être une instance de Wikidata :  
...
Dans une seconde requête, on liera que l'entité trouvée comme ([P170](https://www.wikidata.org/wiki/Property:P170){target="_blank"})-créateur·rice des œuvres que l'on cherche à localiser dans l'espace et le temps.</br>
```{r, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
...
Le résultat renvoyé par cette requête nous permet de constater que le peintre **Johannes Vermeer est directement identifié par une URI (*entity ID*)** [`wd:Q41264`](https://www.wikidata.org/wiki/Q41264) **dans Wikidata**.
<br />
...
```{r, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/modele_interroge_qualifier.png')
```
<center>
