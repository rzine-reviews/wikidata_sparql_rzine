...
write.csv(x = data3_WQSR, file = "data/data.csv", row.names = FALSE)
# Import du fichier de données enregistrées
data3_WQSR <- read.csv("data/data.csv", row.names = NULL)
```
...
Nous pouvons utiliser la fonction `plot()` associée à la fonction `st_geometry()` du package `sf` pour afficher les données dans le fenêtre graphique :
...
**Cela permet d'ajouter le code ISO3 du pays de localisation ("ADM0_A3") à la couche géographique des musées :**
...
Nous pouvons réaliser une représentation graphique de la distibution spatiale des musées. Pour cela, nous calculons le nombre total d'œuvres exposées par pays :
...
Plusieurs informations collectées peuvent être cartographiées. Voici un exemple de carte interactive réalisée avec le package `tmap`, qui représente le nombre de jours d'exposition du tableau dans les différents musées. 
...
**Affichage du tableau *data_semaine* crée :**
...
Bien que ce tableau possède une colonne `geometry`, il s'agit d'un simple `data.frame`. Nous le convertissons en objet `sf` (couche géographique).
...
<b> L'installion du package `av` est nécessaire pour créer des vidéos avec la fonction `tmap_animation()`. Cette fonction permet également de créer des GIFs animés, mais il vous sera nécessaire d'installer le package `gifski` pour profiter de cette fonctionnalité</b>
.</div>    
