...
author: 
 - name: Raphaëlle Krummeich 
   affiliation: UMR IDEES, université de Rouen Normandie
 - name: Hugues Pecout
...
Cet article ou notebook présente une exploration du graphe Wikidata (cf. [parties 1.2](#standard-rdf-du-w3c) et [3](#exploration-du-graphe-wikidata)) et le traitement des données extraites (cf. [parties 2](#packages-et-données) et [4](#exploration-spatiale)). 

Du requêtage en langage SPARQL (cf. [partie 1.3](#le-langage-sparql)) au traitement et à la représentation des données issues du graphe Wikidata, il présente, étape par étape, l'ensemble de la chaîne de traitement réalisée. Le processus exploratoire, parfois itératif, a été conservé afin de mieux comprendre l'approche réflexive des auteurs et autrice. Ce *notebook* retranscrit les démarches scientifique et méthodologique adoptées.

Le schéma organisant ces données est esquissé dans la [partie 1](#wikimédia-wikidata-w3c). Afin de permettre une reproduction des traitements et des résultats présentés, les données automatiquement collectées au `r format(Sys.time(), "%d %B %Y")` et utilisées sont mises à disposition (cf. [partie 2.2](#données-collectées)). L'idée de départ conduit à la démarche suivante en 3 étapes : 
...
Le RDF (*Resource Data Framework*) est un cadre général de modélisation utilisé pour décrire formellement les ressources du Web^[La notion de **ressource Web** peut être définie par la spécification de la norme RFC 3986 : *Uniform Resource Identifier (URI): Generic Syntax* (https://datatracker.ietf.org/doc/html/rfc3986#section-1.1) comme suit : "La présente spécification ne limite pas la portée de ce qui pourrait être une ressource ; le terme "ressource" est plutôt utilisé dans un sens général pour tout ce qui pourrait être identifié par un URI. Des exemples familiers familiers : un document électronique, une image, une source d'information ayant un objectif cohérent (par exemple, "le bulletin météorologique du jour pour Los Angeles"), un service (par exemple, une passerelle HTTP-SMS) et une collection d'autres ressources. Une ressource n'est pas nécessairement accessible via l'internet ; par exemple, les êtres humains, les entreprises et les livres reliés d'une bibliothèque peuvent également être des ressources. De même, des concepts abstraits peuvent être des ressources, tels que les opérateurs et opérandes d'une équation mathématique, les types d'une relation (par exemple, "parent" ou "employé"), ou des valeurs numériques (par exemple, zéro, un et l'infini)."] via leurs métadonnées afin de permettre le traitement par inférence de telles descriptions. Développé par le W3C^[Le ***World Wide Web Consortium*** (**W3C**) est un organisme de standardisation à but non lucratif fondé en 1994 et chargé de promouvoir la compatibilité des technologies du Web], le RDF est le formalisme de base du [Web sémantique](https://fr.wikipedia.org/wiki/Web_s%C3%A9mantique){target="_blank"}. 
...
Le sujet et l'objet, dans le cas où ce sont des ressources, peuvent être identifiés par un identifiant unique de la ressource (URI^[Un **URI** (*Uniform Resource Identifier*, en français "Identifiant unique de ressource") est une chaîne de caractères qui fait référence à une ressource. Les plus courantes sont les URL, qui identifient une ressource en donnant son emplacement sur le Web. Au contraire, les *Uniform Resource Name* (URN) font référence à une ressource grâce à son nom, dans un environnement donné, par exemple le code ISBN d'un livre.]), une valeur ou être des nœuds anonymes. Le prédicat est nécessairement identifié par un identifiant unique de la ressource. 
...
Classiquement,  pour rechercher dans le vocabulaire DBpedia (dbo) des entités qui ont pour [type](https://www.w3.org/TR/rdf-schema/#ch_type){target="_blank"} [ville](https://dbpedia.org/ontology/city){target="_blank"} dans le vocabulaire RDF (rdf), on peut écrire le triplet sous la forme suivante :
...
- [`wikidataR`](https://cran.r-project.org/web/packages/WikidataR/index.html){target="_blank"}^[[`WikidataR`](https://cran.r-project.org/web/packages/WikidataR/index.html){target="_blank"} est développé par Thomas Shafee, Os Keyes, Serena Signorelli, Alex Lum, Christian Graul et Mikhail Popov (développeur de [`WikidataQueryServiceR`](https://cran.r-project.org/web/packages/WikidataQueryServiceR/index.html){target="_blank"}), membre de la Fondation Wikimédia. Il est maintenu par Thomas Shafee de l'université australienne La Trobe de l'état Victoria.] (et son extension [`WikidataQueryServiceR`](https://cran.r-project.org/web/packages/WikidataQueryServiceR/index.html){target="_blank"}) : permet d'écrire une requête pour interroger le graphe de données Wikidata au moyen d'une interface de programmation d’application (API) pour le service de requêtes Wikidata en langage SPARQL^[ Le [service de collecte des données de Wikidata en langage SPARQL](https://query.wikidata.org/){target="_blank"} offre une solution très complète en première approche (exemples, assistant de requêtes, diversité des représentations, etc.). Toutefois, dans une dynamique d'exploration du graphe Wikidata, l'utilisation de R facilite la compréhension, l'archivage et la reproductibilité des différentes requêtes réalisées. Pour cette raison, nous utilisons le *package* [`WikidataR`](https://www.rdocumentation.org/packages/WikidataR/versions/2.3.3){target="_blank"} ou alternativement [`WikidataQueryServiceR`](https://cran.r-project.org/web/packages/WikidataQueryServiceR/index.html){target="_blank"} dont les fonctionnalités sont plus développées.] ;  
- [`rnaturalearth`](https://cran.r-project.org/web/packages/rnaturalearth/index.html){target="_blank"} : met à disposition les données géographiques de [Natural Earth](https://www.naturalearthdata.com/){target="_blank"} ;  
- [`sf`](https://cran.r-project.org/web/packages/sf/index.html){target="_blank"} : permet la gestion et la manipulation de données géographiques vectorielles ;  
- [`dplyr`](https://cran.r-project.org/web/packages/dplyr/index.html){target="_blank"} : pour la manipulation des données ;     
- [`mapview`](https://cran.r-project.org/web/packages/mapview/index.html){target="_blank"} : pour l'affichage interactif de données géographiques (repose sur la librairie [JavaScript Leaflet](https://leafletjs.com/){target="_blank"}) ;  
- [`tmap`](https://cran.r-project.org/web/packages/tmap/index.html){target="_blank"} : pour la création de cartes thématiques ;  
- [`ggplot2`](https://cran.r-project.org/web/packages/ggplot2/index.html){target="_blank"} : permet la production de graphiques basés sur la grammaire graphique [@wilkinson05] ;  
- [`av`](https://cran.r-project.org/web/packages/av/index.html){target="_blank"} : génère des vidéos à partir d'images ou de graphiques R ;  
- [`patchwork`](https://cran.r-project.org/web/packages/patchwork/index.html){target="_blank"} : permet de combiner plusieurs graphiques `ggplot2` dans la fenêtre graphique ;  
- [`jpeg`](https://cran.r-project.org/web/packages/jpeg/index.html){target="_blank"} : permet l'import, l'enregistrement et l'affichage des images matricielles (format bitmap) ;  
...
Les informations sur le système d"exploitation et les versions des *packages* utilisés pour générer ce *notebook* sont disponibleq en <a href="#info-session">fin de document</a>.   
<br />
...
3) des fonds de carte accessibles via [Leaflet](https://rstudio.github.io/leaflet/basemaps.html){target="_blank"}.         
...
<div class="alert alert-success" role="alert">Wikidata est un graphe de données ouvertes et liées, régulièrement mis à jour. Une requête réalisée à deux dates différentes est susceptible de renvoyer un résultat différent.</div> 
<br/>
...
- la clause [`SELECT `]((https://www.w3.org/TR/sparql11-query/#select){target="_blank"}) des données interrogées renvoie les variables spécifiques (avec `COUNT`, qui renvoie le nombre des ces données ou avec `GROUP_CONCAT`, qui les concatène au sein d'une collection, regroupées par `GROUP BY` et triées par `ORDER BY`),  
...
```{r schemasparql, out.width="50%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3Fidentifiant%20%0A%20%20%20%20%20%20%20%20%20%20WHERE%0A%20%20%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%3Fidentifiant%20wdt%3AP31%20wd%3AQ5%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20wdt%3AP373%20%27Johannes%20Vermeer%27.%0A%20%20%20%20%20%20%20%20%20%20%7D)*
</center>
</br> 
...
- par l'identifiant uniforme de ressource [https://www.wikidata.org/wiki/Q41264](https://www.wikidata.org/wiki/Q41264){target="_blank"} pour `WikidataQueryServiceR`.   


Avec `WikidataR`, on obtient une valeur de la base de données sous la forme d'une chaîne de caractères commençant par Q suivi d'un nombre à plusieurs chiffres. 

En utilisant `WikidataQueryServiceR`, on obtient un identifiant uniforme de ressource sur un réseau qui adopte une syntaxe associée à la norme d'internet. Un URI permet d'identifier une ressource de manière pérenne (même si celle-ci est déplacée ou supprimée).</div>
...
```{r schemasparql2, out.width="50%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_2.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3Foeuvre%20%3Fmusee%20%3Fcoord%20WHERE%20%0A%20%20%20%20%20%20%20%20%20%20%20%7B%20%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%3Foeuvre%20wdt%3AP170%20wd%3AQ41264%20.%20%0A%20%20%20%20%20%20%20%20%20%20%20%20%20OPTIONAL%20%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%7B%20%20%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%3Foeuvre%20wdt%3AP276%20%3Fmusee%20.%20%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%3Fmusee%20wdt%3AP625%20%3Fcoord%20%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%7D%20%0A%20%20%20%20%20%20%20%20%20%20%20%7D)*
</center>
</br> 
...
```{r inscription des données sur les œuvres 1,  eval=TRUE, echo=FALSE, include=FALSE}
...
<div class="alert alert-success" role="alert">Les coordonnées géographiques renseignées dans Wikidata sont exclusivement exprimées dans le [système de coordonnées géographiques **WGS84**](https://fr.wikipedia.org/wiki/WGS_84){target="_blank"} (cf. [propriété 625](https://www.wikidata.org/wiki/Property_talk:P625){target="_blank"}).</div>
...
```{r schemasparql3, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_3.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3FoeuvreLabel%20%3FmuseeLabel%20%3Fcoord%20WHERE%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%3Foeuvre%20wdt%3AP170%20wd%3AQ41264%20.%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20OPTIONAL%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%3Foeuvre%20wdt%3AP276%20%3Fmusee%20.%20%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%3Fmusee%20wdt%3AP625%20%3Fcoord%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20SERVICE%20wikibase%3Alabel%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20bd%3AserviceParam%20wikibase%3Alanguage%20%22%5BAUTO_LANGUAGE%5D%2C%20en%22.%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7D)*
</center>
</br> 

Assignation de la requête dans R sous forme de chaîne de caractères dans un objet et exécution de la requête :

```{r requete sparql œuvres de Vermeer et musées, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
```{r inscription des données sur les œuvres,  eval=TRUE, echo=FALSE, include=FALSE}
...
```{r schemasparql4, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_4.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%20%3Foeuvre%20%3FoeuvreLabel%20%0AWHERE%0A%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%3Foeuvre%20wdt%3AP170%20wd%3AQ41264%0A%20%20%20%20%20%20%20SERVICE%20wikibase%3Alabel%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20bd%3AserviceParam%20wikibase%3Alanguage%20%22%5BAUTO_LANGUAGE%5D%2C%20en%20%22.%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%7D%0AORDER%20BY%20DESC%28%3Foeuvre%29)*
</center>
</br> 

Assignation de la requête dans R sous forme de chaîne de caractères dans un objet et exécution de la requête :

```{r requete sparql nombre d œuvres attribuées à Vermeer en valeur simple, eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, message=FALSE}
...
```{r Nombre œuvres associées avec une valeur simple, eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, message=FALSE}
...
```{r schemasparql5, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_5.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3Foeuvre%20%3FoeuvreLabel%20%0AWHERE%0A%20%20%20%20%20%7B%0A%20%20%20%20%20%3Foeuvre%20p%3AP170%2Fps%3AP170%20wd%3AQ41264%20%20%0A%20%20%20%20%20SERVICE%20wikibase%3Alabel%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20bd%3AserviceParam%20wikibase%3Alanguage%20%22%5BAUTO_LANGUAGE%5D%2C%20en%20%22.%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%7D%20%0A%20%20%20%20%20%7D%20%0AORDER%20BY%20DESC%28%3Foeuvre%29)*
</center>
</br> 
...
```{r nombre d œuvres attribuées à Vermeer par asertion, eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, message=FALSE}
...
```{r identification des œuvres attribuées par assertion et non par valeur simple, eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, message=FALSE}
...
ORDER BY DESC(?count)
```
```{r schemasparql6, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_6.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3FoeuvreLabel%20%28COUNT%28DISTINCT%3Fcoord%29%20AS%20%3Fcount%29%20%0AWHERE%0A%20%20%7B%0A%20%20%20%3Foeuvre%20p%3AP170%2Fps%3AP170%20wd%3AQ41264%20%3B%20wdt%3AP276%20%3Fmusee%20.%0A%20%20%20%3Fmusee%20wdt%3AP625%20%3Fcoord%0A%20%20%20SERVICE%20wikibase%3Alabel%0A%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20bd%3AserviceParam%20wikibase%3Alanguage%20%22%5BAUTO_LANGUAGE%5D%2C%20en%20%22.%0A%20%20%20%20%20%20%20%20%7D%20%0A%20%20%20%7D%0AGROUP%20BY%20%3FoeuvreLabel%0AORDER%20BY%20DESC%28%3Fcount%29)*
</center>
</br> 

Assignation de la requête dans R sous forme de chaîne de caractères dans un objet et exécution de la requête :

```{r requete sparql identification du nombre de localisation par œuvre en valeur simple, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
```{r result identification du nombre de localisation par œuvre en valeur simple, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
ORDER BY DESC(?count)
```
```{r schemasparql7, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_7.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3FoeuvreLabel%20%28COUNT%28DISTINCT%3Fcoord%29%20AS%20%3Fcount%29%20%0AWHERE%0A%20%20%7B%0A%20%20%20%3Foeuvre%20p%3AP170%2Fps%3AP170%20wd%3AQ41264%20%3B%20p%3AP276%2Fps%3AP276%20%3Fmusee%20.%0A%20%20%20%3Fmusee%20wdt%3AP625%20%3Fcoord%0A%20%20%20SERVICE%20wikibase%3Alabel%0A%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20bd%3AserviceParam%20wikibase%3Alanguage%20%22%5BAUTO_LANGUAGE%5D%2C%20en%20%22.%0A%20%20%20%20%20%20%20%20%7D%20%0A%20%20%20%7D%0AGROUP%20BY%20%3FoeuvreLabel%0AORDER%20BY%20DESC%28%3Fcount%29)*
</center>
</br> 

Assignation de la requête dans R sous forme de chaîne de caractères dans un objet et exécution de la requête :

```{r requete sparql identification du nombre de localisation par œuvre par assertion, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE}
query <- "SELECT ?oeuvreLabel \
                  (COUNT(DISTINCT?coord) AS ?count) WHERE \
          { \
              ?oeuvre p:P170/ps:P170 wd:Q41264 ; p:P276/ps:P276 ?musee . \
...
```{r result2 sparql identification du nombre de localisation par œuvre par assertion, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE} 
...
#### La circulation spatio-temporelle de l'œuvre
...
```{r schemasparql8, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_8.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3FmuseeLabel%20%3Fdated%20%3Fdatef%20%3Fcoord%20%0AWHERE%20%20%0A%20%20%20%20%7B%0A%20%20%20%20%20%20wd%3AQ4660880%20p%3AP276%20%3Flieu%20.%0A%20%20%20%20%20%20%3Flieu%20ps%3AP276%20%3Fmusee%20%3B%20%0A%20%20%20%20%20%20%20%20%20%20%20%20pq%3AP580%20%3Fdated%20%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20pq%3AP582%20%3Fdatef%20.%0A%20%20%20%20%20%20%3Fmusee%20wdt%3AP625%20%3Fcoord%20.%0A%20%20%20%20%20%20SERVICE%20wikibase%3Alabel%0A%20%20%20%20%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20bd%3AserviceParam%20wikibase%3Alanguage%20%22%5BAUTO_LANGUAGE%5D%2C%20en%20%22.%0A%20%20%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%7D)*
</center>
</br> 

Assignation de la requête dans R sous forme de chaîne de caractères dans un objet et exécution de la requête :

```{r requete sparql identification de la trajectoire spatio-temporelle de l œuvre choisie,  eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, message=FALSE , out.width="50%",fig.align = 'center', purl = FALSE}
...
```{r schemasparql9, out.width="50%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/query_highlight_wikidata_9.png')
```  

<center>
*Schéma : Image de la requête saisie dans le service de requête de wikidata accessible ici : [https://query.wikidata.org/](https://query.wikidata.org/#SELECT%20%3Fimage%20%0AWHERE%0A%20%20%20%20%20%7B%0A%20%20%20%20%20%20wd%3AQ4660880%20wdt%3AP18%20%3Fimage%0A%20%20%20%20%20%7D)*
</center>
</br> 
...
```{r enregistrement de l image collectée de l œuvre,  eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE, message=FALSE}
...
Dans cette partie, nous allons uniquement nous intéresser à la dimension spatiale des données collectées, sans prendre en compte la dimension temporelle. Si aucune coordonnée géographique n'est renseignée (cf. [partie 3.3](#lister-les-œuvres-du-peintre-et-trouver-leurs-localisations-géographiques)), nous ne prenons pas en compte le musée recensé. En revanche, nous considérons l'ensemble des localisations comportant des coordonnées géographiques, peu importe la temporalité qui leur est associée.
...
Plusieurs œuvres semblent avoir été exposées dans les mêmes lieux. Afin de cartographier les musées ayant exposé différentes œuvres de Johannes Vermeer, nous réalisons un regroupement des œuvres par intitulé des musées (*musee*), tout en calculant le nombre d'œuvres que chacune de ces institutions a exposées au cours du temps. Pour cette opération, nous utilisons [l'opérateur *pipe*](https://larmarange.github.io/guide-R/manipulation/pipe.html#le-pipe-natif-de-r){target="_blank"} (`|>`) qui permet d'enchaîner plusieurs fonctions de manière lisible.
...
Nous commençons par télécharger un fond de carte pays mis à disposition par le *package* [`rnaturalearth`](https://cran.r-project.org/web/packages/rnaturalearth/){target="_blank"} :
...
La couche géographique des musées ('musee_geo') et le fond de carte du monde ('monde') sont géoréférencés dans le même système de coordonnées^[Un **système de coordonnées** est un référentiel dans lequel on peut représenter des éléments dans l'espace. Ce système permet de se situer sur le globe terrestre grâce à un couple de coordonnées géographiques.] ([WGS84](https://fr.wikipedia.org/wiki/WGS_84){target="_blank"}). Nous pouvons alors réaliser une jointure spatiale^[Une **jointure spatiale** associe des champs (variables) et des valeurs attributaires d’une couche SIG à une table d’une autre donnée géographique. L’association entre la table attributaire de la couche source et la table jointe se réalise en fonction de la relation spatiale entre les différentes géométries des deux vecteurs concernés. La clé de jointure n’est pas un caractère mais une relation géographique entre les éléments localisés.] qui permet de récupérer pour chaque musée le [code ISO3](https://fr.wikipedia.org/wiki/ISO_3166-1){target="_blank"} du pays dans lequel il est situé. Pour cela, nous utilisons la fonction `st_intersection` du *package* `sf`.
...
```{r nombre d œuvres par_pays,  eval=TRUE, echo=TRUE, include=TRUE}
...
```{r graphe du nombre d œuvres par pays,  eval=TRUE, echo=TRUE, include=TRUE}
...
```{r georéférencement des musées 2,  eval=TRUE, echo=FALSE, include=TRUE}
...
Attention, la création d'un GIF ou d'une vidéo peut prendre plusieurs minutes. L'installation du *package* `av` est nécessaire pour créer des vidéos avec la fonction `tmap_animation()`. Cette fonction permet également de créer des GIFs animés, mais il vous sera nécessaire d'installer le *package* `gifski` pour profiter de cette fonctionnalité.</div>    
...
### Frise chronologique avec `geom_segement()`
...
<div class="alert alert-success" role="alert">
Vous pouvez retrouver l'identifiant Wikidata d'un·e artiste peintre sur la page [Wikidata](https://www.wikidata.org/wiki/Wikidata:Main_Page){target="_blank"}. Indiquez le nom de l'artiste peintre dont vous souhaitez localiser les œuvres dans le champ de recherche. Veillez à bien récupérer l'identifiant correspondant à l'entité "artiste".</div>
...
ID <- "Q41406"
NOM <- "Edvard Munch"
# Sélectionner les noms des œuvres et musées et leurs coordonnées telles que
#  { 
#   l’œuvre a pour peintre l’entité_peintre ;              # chemin principal
#     en option
#      {
#             l’œuvre est localisée dans un musée .        # chemin optionnel
#             ce musée a pour localisation les coordonnées  # chemin optionnel
#      }
...
ggplot(data = by_country, aes(x = ADM0_A3, y = nb_oeuvre) )  +
  geom_bar(stat="identity") +
  ggtitle(paste0("Nombre d'œuvres de ", NOM ," exposé, par pays")) +
  xlab("") +
  ylab("") +
  theme(legend.position = "none",
        axis.text.x = element_text(size=8),
...
En interrogeant la provenance (`?url`) de l'information sur la "Leiden Collection", c'est-à-dire la source fournie (ou non) par le ou la contributrice, on trouve le [lien hypertexte](https://rkd.nl/nl/explore/images/62857){target="_blank"} de l'institut néerlandais d'histoire de l'art. 
