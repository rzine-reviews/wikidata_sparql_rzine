---
title: "Histoire de cadre spatio-temporelle"
subtitle: "Exemple appliqué d'un notebook avec R"
date: "`r Sys.Date()`"
author: 
...
>
> Il permet ainsi de présenter clairement, étapes par étapes, toute une chaîne de traitement de données réalisée avec R à un public qui ne maîtrise pas forcément ce langage. 
>
...
Nous présentons dans ce document une chaîne complète de traitement de données, de leur collecte à leur visualisation graphique. 
...
Dans cet exemple, les données utilisées sont celles disponibles et mises en jour sur Wikidata. Elles sont structurées sous une forme décrite sommairement ci-dessous. Pour plus d'informations, il est possible de consulter l'[introduction à Wikidata](https://www.wikidata.org/wiki/Wikidata:Introduction/fr){target="_blank"} ou la [page Wikibase](https://www.mediawiki.org/wiki/Wikibase/Indexing/RDF_Dump_Format){target="_blank"} dédiée au format RDF du dépôt de données Wikidata.
...
L'idée sous-tendue à la démarche proposée est d'interroger un dépôt de données afin de proposer de tracer une ou des trajectoires. Compte-tenu du formalisme et du contenu des données non supervisées de Wikidata, le choix de l'objet étudié ici est une peinture sur un support physique dans un cadre, susceptible d'être exposée au sein d'un musée ou d'une institution culturelle dans le monde. ([voir les données utilisées](#données-utilisées)).

<br />

# R, code source et packages

**Ce document peut être reproduit sur votre machine**. **Pour cela vous devez avoir installé R** et l'IDE Rstudio (fortement conseillé). Vous devez également installer deux packages^[La bibliothèque des fonctions internes de R est divisée en ensembles de fonctions et de jeux de données apparentés nommés packages (terme que l’équipe de traduction française de R a choisi de conserver tel quel) [(Goulet, 2016, p.62)](https://cran.r-project.org/doc/contrib/Goulet_introduction_programmation_R.pdf){target="_blank"}] (`knitr` et `rmdformats`) pour reproduire ce document à l'identique.

## Installer R et Rstudio

Seul R est indispensable pour reproduire ce document. Mais il est fortement conseillé aux utilisateurs d'installer également l'IDE Rstudio qui facilite énormément la pratique de R et la production de Rmarkdown.
...
- `rmdformats` : met à disposition le modèle de mise en page HTML utilisé (*readthedown*) 

**Installez-les, en exécutant cette ligne de commande dans la console :**
...
<b>Sur Windows, l'exécution d'un fichier Rmarkdown (avec `knitr`) nécessite l'installation du logiciel [<b>Pandoc</b>](https://pandoc.org/){target="_blank"}</b>, qui permet de convertir un fichier d'un format de balisage à un autre (.md vers .html par exemple).
</div>
...
Pour les utilisateurs et les utilisatrices de [logiciel de gestion de versions](https://fr.wikipedia.org/wiki/Logiciel_de_gestion_de_versions){target="_blank"}, vous pouvez cloner le dépôt du code source :   
...
Sinon, vous pouvez récupérer le code source de ce document Rmarkdown en cliquant tout simplement sur ce lien ci-dessous. Décompressez le dossier, puis double-cliquez sur le fichier projet ***Rmarkdown.Rproj***. Cela enclenchera l'ouverture du projet dans Rstudio.
...
Voici la liste des packages utilisés pour l'exploration de données présentée dans ce document. **Il seront automatiquement installés si vous exécutez** (***knitez***) **le fichier source** (***mon_notebook_final.rmd***).
...
- `sf`: pour la gestion et la manipulation (géotraitement) de données géographiques   
- `mapview`: pour produire facilement des cartes interactives (leaflet)   
- `tmap` : pour produire de cartes thématiques
- `ggplot2`: pour produire des graphiques, basés sur la [grammaire graphique théorisée par *Leland Wilkinson*](https://www.cultura.com/the-grammar-of-graphics-9780387987743.html){target="_blank"}    
- `patchwork`: permet de combiner facilement plusieurs graphiques ggplot2   
- `jpeg`: pour importer, enregistrer et afficher des images matricielles (bitmap) 
- `DT`: pour une mise en forme dynamique (HTML) de tableaux de données (bibliothèque javascript [DataTables](https://datatables.net/){target="_blank"}) 
...
<b>Retrouvez l'ensemble des informations système et version de package utilisés en</b> <a href="#info-session"><b>fin de document</b></a>.
</div>    
