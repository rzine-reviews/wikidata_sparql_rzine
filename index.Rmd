...
nocite: |
  @*
link-citations: true
github: "HuguesPecout/sparql_test"
...
> Cette fiche est le fruit d'une collaboration interdisciplinaire de membres du [GT Notebook](https://gitlab.huma-num.fr/gt-notebook) dans le cadre d'un atelier proposé aux [*Journées d’étude normande sur les données de la recherche*](https://www.unicaen.fr/evenement/journee-detude-normande-sur-les-donnees-de-la-recherche/) qui se sont déroulées en décembre 2021. L'association de plusieurs compétences et disciplines a abouti à ce notebook présentant une analyse exploratoire reproductible de données collaboratives wikidata, mises à disposition au format RDF. Les enjeux d'un tel processus inter-disciplinaire sont esquissés en conclusion.
...
Les informations saisies dans Wikidata sont des données “brutes” multilingues non-supervisées :
...
<center>
Figure 1 : **Données Wikipédia (DBpédia) VS données Wikidata**</br>*source : [Wikidata-Basics/Wikidata Hackathon event for the Festival of Creative Learning 2018](https://thinking.is.ed.ac.uk/wikidata-basics/faq-and-further-reading/)*
</center>
...
Il existe plusieurs méthodes pour accéder à ces données. Dans ce document, nous présentons l'accès à la base de données wikidata mises à disposition au **format RDF**. 

**Resource Description Framework** (RDF) est cadre général de modélisation utilisé pour décrire formellement les ressources Web et leurs métadonnées, afin de permettre le traitement automatique de telles descriptions. Développé par le World Wide Web Consortium^[Le **World Wide Web Consortium**, abrégé par le sigle **W3C**, est un organisme de standardisation à but non lucratif, fondé en octobre 1994 chargé de promouvoir la compatibilité des technologies du World Wide Web telles que HTML5, HTML, XHTML, XML, RDF, SPARQL, CSS, XSL, PNG, SVG, MathML et SOAP.], RDF est le formalisme de base du [Web sémantique](https://fr.wikipedia.org/wiki/Web_s%C3%A9mantique). 

Un document structuré en RDF est un ensemble de triplets^[Un **triplet** est un groupe formé par trois éléments dont chacun appartient à un ensemble distinct.]. Un triplet RDF est une association (sujet, prédicat, objet) :
...
<center>
Figure 2 : **Exemple de graphe de triplets**</br>*source : https://www.w3.org/TR/rdf11-primer/*
</center>
...
Par exemple, l'*assertion ou déclaration "Bob est intéressé par Mona Lisa"* est formalisée de la manière suivante :
...
<center>
Figure 3 :  **Composantes génériques d'un triplet RDF**</br>*source : https://www.w3.org/TR/rdf11-primer/*
</center>

</br>

Le sujet et l'objet, dans le cas où ce sont des ressources, peuvent être identifiés par un URI^[Un **URI** (*Uniform Resource Identifier*, en français *Identifiant de ressource uniforme*) est une chaîne qui fait référence à une ressource. Les plus courantes sont les URL, qui identifient une ressource en donnant son emplacement sur le Web. Au contraire, les URN font référence à une ressource grâce à son nom, dans un environnement donné, par exemple le code ISBN d'un livre.] ou être des nœuds anonymes. Le prédicat est nécessairement identifié par un URI.
...
Les packages utilisés pour réaliser l'ensemble de la chaîne de traitement présenté dans ce document sont :
...
2) Couche géographique [Natural Earth](https://www.naturalearthdata.com/) ;      
3) Fonds de carte mis à disposition par [Leaflet](https://rstudio.github.io/leaflet/basemaps.html).         
...
<p class="center">[<span style="font-size: 230%;" class="glyphicon glyphicon-download-alt"></span> <br/> Télécharger les données](https://github.com/HuguesPecout/sparql_test/raw/main/data.zip)</p>
...
`WikidataQueryServiceR` a été développé par Mikhail Popov, membre de la Fondation Wikimédia. Il s'est inspiré des packages [`WikipediR`](https://cran.r-project.org/web/packages/WikipediR/index.html) et [`WikidataR`](https://cran.r-project.org/web/packages/WikidataR/index.html). 
...
L'objectif de cette analyse consiste à s'intéresser à un·e peintre et à ses peintures. Dans Wikidata, les peintres, peintures et lieux d'exposition sont modélisés selon la représentation schématique ci-dessous (Figures 4 et 5), comprenant des entités liées entre elles et mobilisant [une syntaxe spécifique avec des préfixes^[Un **préfixe** est une modalité de simplification d'écriture faisant référence à un *espace de noms*, voir notamment, les préfixes wikidata [ici](https://www.wikidata.org/wiki/EntitySchema:E49){target="_blank"}]](https://www.wikidata.org/wiki/EntitySchema:E49).

Après plusieurs exploratorations, nous avons décidé de nous intéresser au peintre néerlandais du 17e siècle, **Johannes Vermeer**. Dans Wikidata, ce peintre est une entité (préfixe `wd`) :

  - ayant pour propriété directe (préfixe `wdt`) d'être une instance de Wikidata ([P31](https://www.wikidata.org/wiki/Property:P31)) de nature [Q5](https://www.wikidata.org/wiki/Q5) (humain);
  - ayant pour catégorie commune ([P373](https://www.wikidata.org/wiki/Property:P373)) le libellé "Johannes Vermeer";
  - recensée comme créateur·rice ([P170](https://www.wikidata.org/wiki/Property:P170){target="_blank"}) d'œuvres. 
...
Dans Wikidata, cette œuvre est désignée par le biais de l'entité (`wd`) associée à un identifiant unique : [`wd:Q4660880`](https://www.wikidata.org/wiki/Q4660880).

La localisation d'une entité est déclarée au moyen d'une propriété ([P276](https://www.wikidata.org/wiki/Property:P276)), qui permet l'accès à :   

- Une localisation de l'œuvre dans un lieu (géographique et institutionnel) : `wdt:P276` 
- Ou à un ensemble de localisation renseignées dans le temps : `p:P276` associée aux [*qualifiers*](https://www.wikidata.org/wiki/Help:Qualifiers) de la déclaration de lieu (préfixe `pq`) donnant une période temporelle (date de début : [P580](https://www.wikidata.org/wiki/Property:P580) et date de fin : [P582](https://www.wikidata.org/wiki/Property:P582)) ;
...
<center>
Figure 5 : **Classes et propriétés spécifiques de Wikidata mobilisées dans le cas d'une déclaration (*statement*)**
</center>
...
Nous souhaitons interroger la base Wikidata concernant le peintre Johannes Vermeer, instance de wikidata (classe d'objet [P31](https://www.wikidata.org/wiki/Property:P31)), qui est un humain [Q5](https://www.wikidata.org/wiki/Q5) et qui répond au libellé ([P373](https://www.wikidata.org/wiki/Property:P373){target="_blank"}) "Johannes Vermeer".
...
Le résultat renvoyé par cette requête nous permet de constater que le peintre **Johannes Vermeer est directement identifié par une entité d'identification (*entity ID*)** [**Q41264**](https://www.wikidata.org/wiki/Q41264) **dans Wkikdata**.

<br />

Pour **sauvegarder cette image** :
...
<center>
Figure 6 : **Illustration de wikidata pouridentifier visuellement le peintre Johannes Vermeer**
</center>
...
En second lieu, on propose de lister l'ensemble des œuvres de Johannes Vermeer (entité [Q41264](https://www.wikidata.org/wiki/Q41264)).

La requête ci-dessous demande l'identifiant de (ou des) entité·s (œuvre·s), ayant pour créateur (propriété [P170](https://www.wikidata.org/wiki/Property:P170){target="_blank"}) l'artiste Johannes Vermeer ([Q41264](https://www.wikidata.org/wiki/Q41264){target="_blank"}). Cette requête tente également de récupérer le lieu d'exposition renseigné pour chaque œuvre (propriété [P276](https://www.wikidata.org/wiki/Property:P276)), ainsi que sa localisation géographique (coordonnées géographiques du système de référence *World Geodetic System* ([WGS84](https://www.unoosa.org/pdf/icg/2012/template/WGS_84.pdf)).
...
On constate que les données (non supervisées) concernant le lieu d'exposition sont très hétérogènes. Alors que certaines œuvres n'ont aucun lieu d'exposition indiqué car l'information n'est tout simplement pas disponible, d'autres présentent plusieurs localisations et dans certains cas, plusieurs fois le même musée. Pourquoi ?
...
<center>
Figure 7 : **Diagramme d'une entité Wikidata avec les principaux termes utilisés**</br>*Source : [Wikidata](https://www.wikidata.org/wiki/Wikidata:Introduction)*
</center>
...
Pour connaître les modalités de circulation de l'œuvre dans divers musées et institutions, la requête SPARQL ci-dessous collecte les noms des différents lieux d'exposition ([P276](https://www.wikidata.org/wiki/Property:P276){target="_blank"}), ainsi qu'une dimension spatio-temporel associée, qui est composée de coordonnées géographiques ([P625](https://www.wikidata.org/wiki/Property:P625){target="_blank"}) et des dates de début et de fin ([P580](https://www.wikidata.org/wiki/Property:P580){target="_blank"} & [P582](https://www.wikidata.org/wiki/Property:P582){target="_blank"}) de la période pendant laquelle l'œuvre ([Q4660880](http://www.wikidata.org/entity/Q4660880)) semble avoir été présente dans les différents lieux (figure 7). La requête permet également d'obtenir l'image de l'œuvre ciblée ([P18](https://www.wikidata.org/wiki/Property:P18)).
...
<center>
Figure 8 : ***A Young Woman Seated at the Virginals*** **de Johannes Vermeer, 1670**
</center>
...
nb_oeuvre_by_musee <- setNames(nb_oeuvre_by_musee, c("Coordinates", "Museum", "Nb of paint exposed")) 

# Histogramme de répartition des musées en fonction du nombre d'œuvres de J.V. exposées
hist(nb_oeuvre_by_musee$`Nb of paint exposed`, 
...
A partir de la variable 'coord', il est possible de transformer ce tableau de données géographiques dans un format permettant l'analyse spatiale, où chaque individu/entité (musée) est représenté par un point précisément localisé dans l'espace. Pour cela, nous commençons par créer les colonnes *'long'* et *'lat'* à partir de la colonne *'Coordinates'* :

```{r,  eval=TRUE, echo=TRUE, include=TRUE}
# Suppression des chaînes de caractères "Point(" dans la colonne 'Coordinates'
nb_oeuvre_by_musee$Coordinates <- gsub(pattern = 'Point\\(', replacement = '', 
                                       x = nb_oeuvre_by_musee$Coordinates)

# Suppression des chaînes de caractères ")" dans la colonne 'Coordinates'
nb_oeuvre_by_musee$Coordinates <- gsub(pattern = '\\)', replacement = '',
                                       x = nb_oeuvre_by_musee$Coordinates)

# Création d'un liste comportant un vecteur de coordonnées pour chaque musée
out <- strsplit(nb_oeuvre_by_musee$Coordinates, split = ' ')

# Création de deux nouvelles colonnes comportant les longitudes et latitudes
musee_geo <- cbind(nb_oeuvre_by_musee, do.call(rbind, out))

# Renommage de deux nouvelles colonnes créées
...
Il ne reste plus qu'à **convertir notre tableau en véritable couche géographique (objet de type `sf`), en utilisant les variables 'long' et 'lat'**.
```{r,  eval=TRUE, echo=TRUE, include=TRUE, message=FALSE}
# Package sf, pour la gestion de données géographiques
library(sf)

# Transformation du tableau de données vers une couche géographique
musee_geo <- st_as_sf(musee_geo, coords = c("long", "lat"), crs = 4326)

head(musee_geo)
```
L'objet `musee_geo` est désormais un objet `sf`, que l'on va pourvoir cartographier. Il est très facile d'afficher les points créés (musées) sur une carte interactive, en utilisant le package `mapview`. Cela permet, entre autres, de vérifier la qualité des données et du géoréférencement effectué.
...
tm_basemap() +
tm_shape(musee_geo) +
tm_symbols(size="Nb of paint exposed",
           scale = 4,
           col="red3",
           border.col="white",
           alpha =0.5,
           ...
           border.alpha=0.5,
           id = "Museum",
           popup.vars=c("Nombre d'œuvres :"="Nb of paint exposed" ))
...
**Téléchargement d'un fond de carte pays mis à disposition par le package [`rnaturalearth`](https://cran.r-project.org/web/packages/rnaturalearth/README.html):**
...
**Les attributs des entités (pays)** de la couche géographique :
...
La couche géographique des musées ('musee_geo') et le fond de carte du monde ('world') sont géoréférencés dans le même système de coordonées^[Un **système de coordonnées** est un référentiel dans lequel on peut représenter des éléments dans l'espace. Ce système permet de se situer sur l'ensemble du globe terrestre grâce à un couple de coordonnées géographiques.] (le [WGS84](https://fr.wikipedia.org/wiki/WGS_84)). Cependant, ces deux couches sont dans un système de coordonnées sphérique, non une projection cartographique. En effet, par défaut, le package `sf` repose sur bibliothèque [S2 Geometry](https://s2geometry.io/) pour la gestion de ce type de coordonnées, et cela risque de nous compliquer la tâche pour les traitements spatiaux à venir...

Nous utilisons donc la fonction `sf_use_s2` du package `sf` pour modifier la définition par défaut de ce paramètre. 
```{r,  eval=TRUE, echo=TRUE, include=TRUE, warning = FALSE}
sf_use_s2(FALSE)
```
Nous pouvons ensuite réaliser une [*jointure spatiale*](https://www.emse.fr/tice/uved/SIG/Glossaire/co/Jointure_spatiale.html) qui nous permet d'obtenir pour chaque musée, le [code ISO3](https://fr.wikipedia.org/wiki/ISO_3166-1) du pays dans lequel il se situe. Pour cela, nous utilisons la fonction `st_intersection` du package `sf`.
...
```{r,  eval=TRUE, echo=TRUE, include=TRUE}
nb_oeuvre_musee_geo <- aggregate(Nb.of.paint.exposed ~ ADM0_A3, data = musee_geo, FUN = sum)
```
...
ggplot(data = nb_oeuvre_musee_geo, 
       aes(x = reorder(ADM0_A3, -Nb.of.paint.exposed), y = Nb.of.paint.exposed, fill = ADM0_A3)) +
  geom_bar(stat = "identity") +
  ggtitle("Nombre d'œuvres de Johannes Vermeer exposées, par pays") +
  xlab("") +
  ylab("") +
  scale_fill_manual(values = c("#FC8D62", "#80B1D3"  ,"#E78AC3","#A6D854","#FFD92F","#E5C494",
                               "#FB8072","#BEBADA","#66C2A5","#FFFFB3","#8DA0CB")) +
  annotate(geom="text", 
           x = 9, y = 13, 
...
# Création de deux nouvelles colonnes comportant les longitudes et latitudes
data_geo <- cbind(data, do.call(rbind, out))

# Suppression des colonnes inutiles
data_geo <- data_geo[, c(2:4,6,7)]

# Renommage de deux nouvelles colonnes
colnames(data_geo)[4:5] <- c('long', 'lat')

# Conversion du dataframe en objet sf (couche géographique)
...
nb_expo_musee_geo <- aggregate(x = data_geo[,1], by = list(Musée=data_geo$nomLabel), FUN = length)
```
...
# Popup content
content <- paste0( "<img  src='http://commons.wikimedia.org/wiki/Special:FilePath/Vermeer%20-%20A%20young%20Woman%20seated%20at%20the%20Virginals.jpg'  width='180'><br /><b>",  nb_expo_musee_geo$Musée,
                  "</b><br />Nombre d'expositions de l'œuvre : <b>",nb_expo_musee_geo$nomLabel,"</b>")
# Carte
mapview(nb_expo_musee_geo,
        zcol = "Musée",
        col.regions = "red",
        legend = FALSE,
        popup = content,
...
datatable(st_drop_geometry(data_geo[c(1:4)]))
```
...
Il est ensuite simple de caculer le nombre de jours de présence dans un musée avec une soustraction :

```{r,  eval=TRUE, echo=TRUE, include=TRUE}
data_geo$datef  - data_geo$dated
```
Nous sauvegardons ces valeurs (nombre de jours) dans une nouvelle colonne ('nb_days')

```{r,  eval=TRUE, echo=TRUE, include=TRUE}
data_geo$nb_days <- as.integer(data_geo$datef  - data_geo$dated)
```
...
```{r,  eval=TRUE, echo=TRUE, include=TRUE}
nb_days_musee_geo <- aggregate(x = data_geo$nb_days, by = list(Musée = data_geo$nomLabel), FUN = sum)
```
...
Afin d'obtenir ces variables, nous réalisons une jointure entre les différents tableaux, en utilisant les noms des musées comme clef de jointure.
```{r,  eval=TRUE, echo=TRUE, include=TRUE}

# Jointure entre 'nb_days_musee_geo' et 'nb_expo_musee_geo'
tab_museum <- merge(nb_expo_musee_geo, nb_days_musee_geo, by = "Musée")

# Jointure entre 'tab_museum' et  'nb_expo_musee_geo'
tab_museum <- merge(tab_museum, st_drop_geometry(data_geo[,c(1,4)]), by.x = "Musée", by.y="nomLabel", all.x = FALSE)
...
tm_bubbles("nb_days",
           ...
           id = "Musée",
           popup.vars=c("Nombre de séjours :"="nb_expo",
                        "Nombre total de jours :"="nb_days"))
...
La carte précedente ne permet pas de rendre compte de la trajectoire spatio-temporelle de l'œuvre. Une carte animée représentant la localisation de l'œuvre au cours du temps permettrait de formaliser son déplacement dans l'espace et dans le temps.
...
data_by_week <- data.frame(week = seq(min(data_geo$dated), max(data_geo$datef), "week"),
                           Musée = as.character(NA),
                           Nb_day = as.integer(NA),
...
temp <-   data_geo[data_geo$dated <= data_by_week$week[i] & data_geo$datef >= data_by_week$week[i], c(1,6,4,5)]
...
# Musée NA = "Emplacement inconnu"
data_by_week[is.na(data_by_week$Musée), "Musée"] <- "Emplacement inconnu"

# Pays NA = "pays ?"
data_by_week[is.na(data_by_week$pays), "pays"] <- "pays ?"

# Nombre total de jours d'exposition NA = 0
data_by_week[is.na(data_by_week$Nb_day), "Nb_day"] <- 0


# Affichage du tableau
datatable(data_by_week)
```
...
```{r,  eval=TRUE, echo=TRUE, include=TRUE}
data_by_week <-st_as_sf(data_by_week)
```
...
CARTE  <- tm_shape(world) +                      # Ajout fond de carte monde
          ...
          tm_shape(data_by_week) +               # Ajout des musées, mais...
          tm_symbols(col = "red") +              
  
          tm_facets(along = "week",              # Une carte / semaine grâce à tm_facet
                    free.coords= FALSE, 
                    drop.units=FALSE)  +
      
          tm_layout(main.title ="",              # Gestion du titre
                    panel.label.fontface = 2,
                    panel.labels = paste0("Semaine du ", format(x = data_by_week$week, format = "%d %b %Y" )),
                    bg.color="lightblue") +
    
          tm_credits(bg.color = 'red',           # Affichage Nom des musées et Nombre de jours d'exposition
                     width = 0.5,
                     position = c('center', 'bottom'),
                     text = paste0(data_by_week$Musée, " (",data_by_week$pays,")\n", data_by_week$Nb_day, " jours d'exposition"),
                     size = 1.2,
                     fontface = "bold",
                     col = 'white',
                     align = 'center')
...
Avec la fonction `tm_facet`, ce bout de code crée autant de cartes qu'il y a de lignes dans le tableau `data_by_week`.
...
# Tri en fonction de l'ordre de la liste
...
# Conversion de l'objet sf en data.frame
df_geo <- st_drop_geometry(data_geo)

# Sélection des premières dates d'arrivée (si il en existe plusieurs) pour chaque musée
date_label <- df_geo[!duplicated(df_geo$nomLabel), 1:2]

# Jointure par la date d'arrivée - ajout de la colonne label (nomLabel.y)
data_geo <- merge(data_geo, date_label, by="dated", all.x=TRUE)
...
ggplot(data_geo, aes(x = dated, y = nomLabel.x))
...
- Xend = 'dated' (date d'arrivée) : limite du segment en x ;
...
Mon_graph <- ggplot(data_geo, aes(x = dated, y = nomLabel.x, color = ADM0_A3)) +
                 geom_segment(aes(xend = datef,  yend = nomLabel.x, color =  ADM0_A3), 
                              size = 6)
...
# Suppression des chaînes de caractères "Point(" dans la variable 'coord'
nb_oeuvre$coord <- gsub(pattern = 'Point\\(', replacement = '', nb_oeuvre$coord)

# Suppression des chaînes de caractères ")" dans la variable 'coord'
...
# Création de deux nouvelles colonnes avec longitude et latitude
...
by_country <- aggregate(Nb_oeuvre ~ ADM0_A3 , data = my_museum, FUN = sum)
