...
- [`wikidataR`](https://cran.r-project.org/web/packages/WikidataR/index.html)^[[`WikidataR`](https://cran.r-project.org/web/packages/WikidataR/index.html){target="_blank"} est développé par Thomas Shafee, Os Keyes, Serena Signorelli, Alex Lum, Christian Graul et Mikhail Popov (développeur de [`WikidataQueryServiceR`](https://cran.r-project.org/web/packages/WikidataQueryServiceR/index.html)), membre de la Fondation Wikimédia. Il est maintenu par Thomas Shafee de l'université australienne La Trobe de l'état Victoria.] (et son extension [`WikidataQueryServiceR`](https://cran.r-project.org/web/packages/WikidataQueryServiceR/index.html)) : permet d'écrire une requête pour interroger le graphe de données Wikidata au moyen d'une Interface de Programmation d’Application (API) pour le service de requêtes Wikidata en langage SPARQL^[ Le [service de collecte des données de Wikidata en langage SPARQL](https://query.wikidata.org/){target="_blank"} offre une solution très complète en première approche (exemples, assistant de requêtes, diversité des représentations etc.). Toutefois, dans une dynamique d'exploration du graphe Wikidata, l'utilisation de R facilite la compréhension, l'archivage et la reproductibilité des différentes requêtes spécifiques réalisées. Pour cette raison, nous utilisons le *package* [`WikidataR`](https://www.rdocumentation.org/packages/WikidataR/versions/2.3.3){target="_blank"} ou alternativement [`WikidataQueryServiceR`](https://cran.r-project.org/web/packages/WikidataQueryServiceR/index.html){target="_blank"} qui est un *package* ayant des fonctionnalités plus développées.] ;  
- [`rnaturalearth`](https://cran.r-project.org/web/packages/rnaturalearth/index.html) : met à disposition les données géographiques de [Natural Earth](https://www.naturalearthdata.com/){target="_blank"} ;  
- [`sf`](https://cran.r-project.org/web/packages/sf/index.html) : permet la gestion et la manipulation de données géographiques vectorielles ;  
- [`dplyr`](https://cran.r-project.org/web/packages/dplyr/index.html) : pour la manipulation des données ;     
- [`mapview`](https://cran.r-project.org/web/packages/mapview/index.html) : pour l'affichage interactif de données géographiques (repose sur la librairie [JavaScript Leaflet](https://leafletjs.com/){target="_blank"}) ;  
- [`tmap`](https://cran.r-project.org/web/packages/tmap/index.html) : pour la création de cartes thématiques ;  
- [`ggplot2`](https://cran.r-project.org/web/packages/ggplot2/index.html) : permet la production de graphiques basés sur la grammaire graphique [@wilkinson05] ;  
- [`av`](https://cran.r-project.org/web/packages/av/index.html) : génère des vidéos à partir d'images ou de graphiques R ;  
- [`patchwork`](https://cran.r-project.org/web/packages/patchwork/index.html) : permet de combiner plusieurs graphiques `ggplot2` dans la fenêtre graphique ;  
- [`jpeg`](https://cran.r-project.org/web/packages/jpeg/index.html) : permet l'import, l'enregistrement et l'affichage des images matricielles (format bitmap) ;  
