...
Plusieurs méthodes permettent d'accéder à ces données. Dans ce notebook, nous utilisons un accès au graphe de Wikidata mobilisant quelques éléments du langage de requête [SPARQL](https://www.w3.org/TR/sparql11-query/). L'usage de ce langage de requête spécifique est déployé à l'aide de deux packages : 
...
- et les différents préfixes associés aux spécificités des propriétés simples, directes et indirectes etc. des fragments de graphe de Wikidata ([+d'info](https://linkeddatafragments.org/)). 
<br/>
...
resultat_property
```
Une seule entité de valeur `Q41264` parmi les dix collectées semble correspondre à l'image au sens de [@hui_induction_2015]) de l'objet Johannes Vermeer, le peintre recherché. Le résultat de la requête `find_item` est composé d'un ensemble d'informations (`$id`, `$repository` etc., voir ci-dessous). 
...
## Trouver l'identifiant Wikidata d'un peintre (1ère requête SPARQL)
...
```{r figure5, echo=FALSE, out.width="45%", fig.align = 'center', purl = FALSE }
...
## Lister les œuvres du peintre et trouver leurs localisations géographiques (2nde requête SPARQL)
...
```{r figure 6d, out.width="70%", fig.align = 'center', echo=FALSE, purl=FALSE}
...
Ici, deux sources, l'une issue de la presse spécialisée et l'autre du catalogue d'un musée, permettent de qualifier la nature de la controverse concernant l'attribution de l'œuvre à Johannes Vermeer.<br><br>
...
### Les collections associées aux œuvres de Johannes Vermeer
...
```
Les modalités d'instanciation de ces assertions semblent hétérogènes au sein de Wikidata : en effet, on compte 15 types différents dont seulement 2 se réfèrent au terme collection, la majorité étant de type la valeur [`musée d'art`](https://www.wikidata.org/wiki/Q207694). Afin de cerner à quel type d'instance la propriété associant les œuvres à des collections fait référence, il est possible d'ajouter la description de ces types en utilisant le vocabulaire [schema.org](https://schema.org/) et en concatenant les résultats par type, par exemple :
...
### Choisir l'œuvre de Vermeer qui a le plus voyagé
...
L’œuvre intitulée "Dame assise au virginal" (entité [Q4660880](http://www.wikidata.org/entity/Q4660880)) est, selon Wikidata, l'œuvre de Johannes Vermeer qui a le plus voyagé pour être exposée. Cela signifie que cette instance de la classe œuvres possède le plus grand nombre de valeurs associées au prédicat direct de "lieu" [P276](https://www.wikidata.org/wiki/Property:P276){target="_blank"} dans les déclarations exhaustives de localisation désignant des lieux, dont les coordonnées sont données par le prédicat simple [P625](https://www.wikidata.org/wiki/Property:P625){target="_blank"}.
...
Comme illustré dans les figures 6b et 6e, la localisation spatio-temporelle d'une entité est déclarée au moyen d'un prédicat direct ([p:P276](https://www.wikidata.org/wiki/Property:P276)), qui permet l'accès à une localisation de l'œuvre :
- dans un lieu (géographique et institutionnel) avec la prédicat `ps:P276`, associé à l'ensemble des déclarations de localisations spatiales au moyen du prédicat `wdt:P625`,
- liée dans le temps aux prédicats de type [*qualifiers*](https://www.wikidata.org/wiki/Help:Qualifiers) de la déclaration `?lieu` donnant une période temporelle (date de début : [`pq:P580`](https://www.wikidata.org/wiki/Property:P580), date de fin: [`pq:P582`](https://www.wikidata.org/wiki/Property:P582)) ou date/point dans le temps d'un événement [`pq:P585`](https://www.wikidata.org/wiki/Property:P585).
