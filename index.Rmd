...
author: 
 - name: Raphaëlle KRUMMEICH
   affiliation: IDEES, UMR6266 - université de Rouen Normandie
 - name: Hugues PECOUT
...
> Cette fiche est le fruit d'un dialogue interdisciplinaire de membres du [GT Notebook](https://gitlab.huma-num.fr/gt-notebook) dans le cadre d'un [atelier](https://gt-notebook.gitpages.huma-num.fr/workshop/workshop_3_decembre_2021/slide2html-3-dec/){target="_blank"} proposé aux [*Journées d’étude normande sur les données de la recherche*](https://www.unicaen.fr/evenement/journee-detude-normande-sur-les-donnees-de-la-recherche/) qui se sont déroulées en décembre 2021. L'association de plusieurs compétences et disciplines a abouti à ce notebook présentant une analyse exploratoire reproductible de données ouvertes et liées du graphe *Wikidata*, interrogeables sous la forme de triplets au format RDF. Les enjeux d'un tel processus inter-disciplinaire sont esquissés en conclusion. L'ébauche des requêtes SPARQL élaborées a fait l'objet d'une présentation et d'échanges au sein des [Ateliers du Web Sémantique](https://les-ateliers-du-web-semantique.gitpages.huma-num.fr/les-ateliers-du-web-semantique/) au printemps 2021.

# Introduction {-}

Cette fiche présente une exploration de données du graphe *Wikidata* (cf. [partie 1.2](#standard-rdf-du-w3c)). Du requêtage en langage SPARQL (cf. [partie 3](#interrogation-de-wikidata)) au traitement et à la représentation des données issues du graphe *Wikidata*, ce document présente, étape par étape, l'ensemble de la chaîne de traitement réalisée. Le processus exploratoire des données, parfois itératif, a été conservé. Cela permet de bien comprendre l'approche réflexive des auteurs et autrice et de retranscrire au mieux la démarche scientifique adoptée.

Le schéma organisant ces données est esquissé dans la [partie 1](#wikimédia-wikiproject-w3c). Les données automatiquement collectées (le `r format(Sys.time(), "%d %B %Y")`) et utilisées dans ce document sont mises à disposition ([partie 2.2](#données-collectées)) afin de permettre une reproduction des traitements et des résultats présentés. L'idée de départ qui conduit à la démarche proposée est de requêter les données du graphe *Wikidata*, d'identifier leurs modalités pouvant comporter une dimension spatiale et temporelle et ainsi, de reconstruire une ou des trajectoires dans le temps et l'espace. Compte-tenu du [formalisme](https://www.wikidata.org/wiki/Wikidata:WikiProject_Ontology/Modelling) et de l'hétérogénéïté des données non supervisées de *Wikidata*, l'image de l'objet choisi pour l'étude est une peinture exposée au sein de musées ou d'institutions culturelles dans le monde.

Pour reproduire l'expérimentation, un exemple d'exploration du graphe *Wikidata* pour diverses images d'artistes-peintres est proposée dans la dernière partie. [partie 7](#à-vous-dexplorer).
<br/>
...
```{r figure2, out.width="75%", fig.align = 'center',  echo=FALSE, purl = FALSE }
...
Il existe plusieurs méthodes pour accéder à ces données. Dans ce document, nous utilisons un accès au graphe de *Wikidata* mobilisant le langage de requête [SPARQL](https://www.w3.org/TR/sparql11-query/), qui n'est pas l'objet de la fiche. Ce langage de requête spécifique au cadre de description RDF des données est déployé à l'aide de deux packages. Le package `wikidataR` permet d'écrire la requête dans le langage SPARQL pour le déployer au sein du graphe Wikidata selon ses spécificités, le package `glitter`, actuellement en développement, permet d'écrire ces requêtes pour sonder divers graphes, dont Wikidata, sans connaître la syntaxe SPARQL.

## Le langage SPARQL

SPARQL (prononcé *sparkle*, en anglais : « étincelle ») est un langage de requête et un protocole qui permet de rechercher, d'ajouter, de modifier ou de supprimer des données RDF disponibles à travers Internet. Son nom est un acronyme récursif qui signifie **S**PARQL **P**rotocol **A**nd **R**DF **Q**uery **L**anguage. (@frwiki:194953405)
...
- et les préfixes associés aux fragments de graphe de *Wikidata* ([+d'info](https://linkeddatafragments.org/)). 
<br/>
...
- `WikidataR`: fournit une API client pour le [*Wikidata Query Service*](https://query.wikidata.org/){target="_blank"} (cf. [partie 3](#interrogation-de-wikidata));  
- `glitter` : permet l'écriture et l'envoi de requêtes SPARQL sans en connaître la syntaxe;    
...
Pour installer ces packages, vous pouvez utiliser les lignes de code suivantes, qui vérifient que les packages nécéssaires ne soient pas déjà installés sur votre machine :

```{r installationdespackages1, eval=FALSE, echo=TRUE, include=TRUE, message=FALSE, warning=FALSE}
...
# Pour installer le package `glitter` en cours de développement [mis à disposition sur GitHub](https://github.com/lvaudor/glitter/) :
...
1) Données issues du graphe *Wikidata* ;    
...
<b>*Wikidata* est un graphe de données ouvertes et liées, régulièrement mise à jour. Une requête réalisée à deux dates différentes est susceptible de renvoyer un résultat différent.</b></div>   
<br/>

# Exploration du graphe *Wikidata*

Un [service de collecte des données de *Wikidata* en langage SPARQL](https://query.wikidata.org/) est mis à disposition et offre une solution très complète en première approche (exemples, assistant de requêtes, diversité des représentations etc.). Toutefois, dans une dynamique d'exploration du graphe *Wikidata*, l'utilisation de R facilite l'archivage et la reproductibilité des différentes requêtes réalisées. Pour cette raison, nous utilisons le package [`WikidataR`](https://www.rdocumentation.org/packages/WikidataR/versions/2.3.3) qui fournit une Interface de Programmation d'Application (API) pour interroger le [*dépôt du graphe Wikidata*](https://query.wikidata.org/){target="_blank"} directement via *R* ou alternativement [`WikidataQueryServiceR`]() qui est un package ayant des fonctionnalités plus développées.

[`WikidataR`](https://cran.r-project.org/web/packages/WikidataR/index.html) est développé par Thomas Shafee, Os Keyes, Serena Signorelli, Alex Lum, Christian Graul et Mikhail Popov, membre de la Fondation Wikimédia. Il est maintenu par Thomas Shafee de l'université australienne La Trobe de l'état Victoria. 
<br />

## Modèle des données : des artistes-peintres et trajectoires des oeuvres

Dans *Wikidata*, les peintres, peintures et lieux d'exposition sont modélisés selon les fragments de graphes ci-dessous (Figures 4 et 6.a à c), comprenant des classes instanciées, liées entre elles par des propriétés (prédicats) et mobilisant [une syntaxe spécifique avec des préfixes^[Un **préfixe** est une modalité de simplification d'écriture faisant référence à un *espace de noms*, voir notamment, les préfixes *Wikidata* [ici](https://www.wikidata.org/wiki/EntitySchema:E49){target="_blank"}]](https://www.wikidata.org/wiki/EntitySchema:E49). Le fragment de graphe de données *Wikidata* objet du processus exploratoire est illustré en figure 6b, ci-dessous.
</br>
Après plusieurs explorations, nous avons décidé de nous intéresser au peintre néerlandais du 17e siècle, **Johannes Vermeer** et à l'une de ses oeuvres dont les données spatiales et temporelles semblent suffisamment renseignées.

## Première requête SPARQL

Nous interrogeons le graphe *Wikidata* concernant le peintre Johannes Vermeer, instance de *Wikidata* (classe d'objet [P31](https://www.wikidata.org/wiki/Property:P31)), qui est un humain [Q5](https://www.wikidata.org/wiki/Q5) et qui répond à la catégorie commune ([P373](https://www.wikidata.org/wiki/Property:P373){target="_blank"}) "Johannes Vermeer" selon le schéma simplifié proposé dans la figure 4.

```{r figure4, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
knitr::include_graphics('figures/modele_interroge.png')
```  
<center>
Figure 4 : **schéma des triplets de données d'entrée et de sorties du fragment de graphe Wikidata exploré.**
</center>
</br> 

A l'aide de la fonction `query_wikidata` du package `WikidataR` et des fonctions du package `glitter`, nous commençons dans un premier temps par obtenir l'illustration associée à Johannes Vermeer (renseignée par la propriété [P18](https://www.wikidata.org/wiki/Property:P18)), avec la requête SPARQL suivante :
...
# Package API du CRAN pour interroger le dépôt Wikidata : WikidataR
 library(WikidataR)
# Recherche de l'identifiant de l'artiste nommé 'Johannes Vermeer' et de(s) l'illustration(s) associée(s)
 data1w <- query_wikidata('
           SELECT ?identifiant ?image  WHERE {
               ?identifiant wdt:P31 wd:Q5 ; wdt:P373 "Johannes Vermeer"; wdt:P18 ?image .
               }
 ')
 # Packages API du CRAN pour interroger le dépôt Wikidata : WikidataQueryServiceR
 library(WikidataQueryServiceR)
# Recherche de l'identifiant de l'artiste nommé 'Johannes Vermeer' et de(s) l'illustration(s) associée(s)
 data1ws <- query_wikidata('
           SELECT ?identifiant ?image  WHERE {
               ?identifiant wdt:P31 wd:Q5 ; wdt:P373 "Johannes Vermeer"; wdt:P18 ?image .
               }
 ')
...
```{r affichagedesdonnees,  eval=TRUE, echo=TRUE}
library(DT)
...
Le résultat renvoyé par cette requête nous permet de constater que l'image de l'objet Johannes Vermeer peintre est **identifiée par une entité [Q41264](https://www.wikidata.org/wiki/Q41264) dans *Wikidata* ** et/ou son URI. 
```{r identifiant, eval=TRUE, echo=TRUE, include=TRUE, warning=FALSE}
# Affichage de la valeur de l'URI de la variable entité identifiant collectée avec glitter (ou WikidataQueryService)
data1$identifiant
# Affichage de la valeur de l'entité identifiant collectée avec la package WikidataR
data1ws$identifiant
```
Par ailleurs, on sauvegarde le fichier de l'image numérisée de son portrait en deux étapes à partir de son URI collectée avec la package `glitter` :
...
```
<center>
Figure 5 : **Elément visuel (url vers un fichier .jpg) de Wikidata permettant d'identifier le peintre Johannes Vermeer**
</center>
</br>    
Dans une seconde requête, on liera l'identifiant de l'entité collectée comme ([P170](https://www.wikidata.org/wiki/Property:P170){target="_blank"}) - créatrice des œuvres que l'on cherche à localiser dans l'espace et le temps.</br>
...
```{r figure6, out.width="75%", fig.align = 'center', echo=FALSE, purl = FALSE }
